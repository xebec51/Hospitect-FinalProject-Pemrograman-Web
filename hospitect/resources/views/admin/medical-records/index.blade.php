...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Manajemen Rekam Medis</h1>
    ...
    <!-- Pencarian -->
    <form method="GET" action="{{ route('admin.medical-records.index') }}" class="mb-4">
        <div class="flex items-center">
            ...
            <button type="submit" class="ml-2 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 flex items-center">
                ...
                <thead class="bg-gray-100">
                    <tr>
                        <th class="border border-gray-200 p-2 text-left">
                            <a href="{{ route('admin.medical-records.index', ['sort' => 'record_date', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}" class="flex items-center">
                                Tanggal
                                <i class="ml-2 fas fa-sort{{ request('sort') == 'record_date' ? (request('direction') == 'asc' ? '-up' : '-down') : '' }}"></i>
                            </a>
                        </th>
                        <th class="border border-gray-200 p-2 text-left">
                            <a href="{{ route('admin.medical-records.index', ['sort' => 'patient_name', 'direction' => request('direction') == 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}" class="flex items-center">
                                Pasien
                                <i class="ml-2 fas fa-sort{{ request('sort') == 'patient_name' ? (request('direction') == 'asc' ? '-up' : '-down') : '' }}"></i>
                            </a>
                        </th>
                ...
                <tbody>
                    @foreach ($medicalRecords as $record)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border border-gray-200 p-2">{{ $record->patient->user->name }}</td>
